...
describe.skip("query", function() {
    it("Basic SELECT tests", function() {
      ...
      assert(solution.query().select().execute(), [], 'No FROM clause produces empty array');
      assert(solution.query().from(numbers).execute(), numbers, 'SELECT can be omited');
      assert(solution.query().execute(), []);
      assert(solution.query().from(numbers).select().execute(), numbers, 'The order does not matter');
    });
    ...
    it("Basic SELECT and WHERE over objects", function() {
      ...
      assert(query().select().from(persons).execute(), persons);
      ...
      assert(query().select(profession).from(persons).execute(),  ["teacher","teacher","teacher","scientific","scientific","scientific","politician"]);
      assert(query().select(profession).execute(), [], 'No FROM clause produces empty array');
      ...
      assert(query().select(profession).from(persons).where(isTeacher).execute(), ["teacher", "teacher", "teacher"]);
      
            
      //SELECT * FROM persons WHERE profession="teacher" 
      assert(query().from(persons).where(isTeacher).execute(), persons.slice(0, 3));
      ...
      assert(query().select(name).from(persons).where(isTeacher).execute(), ["Peter", "Michael", "Peter"]); 
      assert(query().where(isTeacher).from(persons).select(name).execute(), ["Peter", "Michael", "Peter"]); 
    ...
    it('GROUP BY tests', function() {
      ...
      assert(query().select(professionGroup).from(persons).groupBy(profession).execute(), ["teacher","scientific","politician"]);
      ...
      assert(query().select(professionCount).from(persons).groupBy(profession).execute(),  [["teacher",3],["scientific",3],["politician",1]]);
      ...
      assert(query().select(professionCount).from(persons).groupBy(profession).orderBy(naturalCompare).execute(), [["politician",1],["scientific",3],["teacher",3]]);
    });
    ...
    it('Numbers tests', function() {
      ...
      //SELECT * FROM numbers
      assert(query().select().from(numbers).execute(), numbers);

      //SELECT * FROM numbers GROUP BY parity
      assert(query().select().from(numbers).groupBy(parity).execute(), [["odd",[1,3,5,7,9]],["even",[2,4,6,8]]]);

      //SELECT * FROM numbers GROUP BY parity, isPrime
      assert(query().select().from(numbers).groupBy(parity, prime).execute(), [["odd",[["divisible",[1,9]],["prime",[3,5,7]]]],["even",[["prime",[2]],["divisible",[4,6,8]]]]]);
      ...
      assert(query().select().from(numbers).groupBy(parity).having(odd).execute(), [["odd",[1,3,5,7,9]]]);      
     ...
     assert(query().select().from(numbers).orderBy(descendentCompare).execute(), [9,8,7,6,5,4,3,2,1]);
      ...
      assert(query().select().from(numbers).where(lessThan3, greaterThan4).execute(),  [1,2,5,6,7,8,9]);
    });
    ...
    it('Frequency tests', function() {
      ...
      assert(query().select(sumValues).from(persons).orderBy(naturalCompare).groupBy(nameGrouping).execute(),  [["Anna",4],["Michael",10],["Peter",10]]);
      ...
      assert(query().select(frequency).from(numbers).groupBy(id).execute(), [{"value":1,"frequency":3},{"value":2,"frequency":2},{"value":3,"frequency":1},{"value":5,"frequency":2},{"value":6,"frequency":2}]);
      ...
      assert(query().select(frequency).from(numbers).groupBy(id).having(greatThan1).having(isPair).execute(),  [{"value":2,"frequency":2},{"value":6,"frequency":2}]);
    ...
    it('join tests', function() {
      ...
      assert(query().select(student).from(teachers, students).where(teacherJoin).execute(), [{"studentName":"Michael","teacherName":"Peter"},{"studentName":"Rose","teacherName":"Anna"}]);
      
      
      var numbers1 = [1, 2];
      var numbers2 = [4, 5];

      assert(query().select().from(numbers1, numbers2).execute(), [[1,4],[1,5],[2,4],[2,5]]);
      ...
      assert(query().select(student).from(teachers, students).where(teacherJoin).where(tutor1).execute(), [{"studentName":"Michael","teacherName":"Peter"}]);
